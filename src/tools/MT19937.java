...
//                                                     // 第1パラメータ：中心値μ
//                                                     // 第2パラメータ：標準偏差σ
// double CauchyRandomNumber   = MT19937.Cauchy(0,1)  // // 中心値０、広がり１のコーシー分布
//                                                     // 第1パラメータ：中心値μ
//                                                     // 第2パラメータ：広がりσ
//      f(x) = \frac{1}{\pi \sigma} \frac{1}{1 + \frac{(x - \mu)^2}{\sigma^2}}
// 
...
/**
 * Java porting from a C-program for MT19937, originaly 
    ...
    **/
    public static double nextDouble() {
        ...
        if (mti >= N) {
            ...
            for (;kk<N-1;kk++) {
                y = (mt[kk]&UPPER_MASK)|(mt[kk+1]&LOWER_MASK);
                mt[kk] = MASK_32BIT(mt[kk+(M-N)] ^ (y >>> 1) ^ mag01[(int)(y & 0x1L)]);
            }
	...
	//-------一様分布乱数のクラス
	public static double uniform(){return(nextDouble());}

	//-------正規分布乱数のクラス
	public static double Gaussian( double average, double standardDeviation ){ 
		double r1, r2, s; // 標準正規分布乱数発生のための変数
		//-------------------標準正規分布乱数発生
		double normal = nextDouble() + nextDouble() + nextDouble() + nextDouble() + nextDouble() + nextDouble();
		normal += (nextDouble() + nextDouble() + nextDouble() + nextDouble() + nextDouble() + nextDouble() );
		normal -= 6 ;
		//---------------------------
	...
	//-------正規分布乱数のクラス
	public static double Cauchy( double mean, double dev ){ 
		double x = 0, y = 0; // 標準Cauchy分布乱数発生のための変数
		//-------------------分布乱数発生
		do{
			x = 1 - nextDouble() ;
			y = 2 * nextDouble() - 1 ;
...
//		MT19937 drand = new MT19937() ;
		...
		System.out.println( MT19937.Gaussian(0,1) );
		System.out.println( "Gaussian (normal) distribution N(0,1)" );
		System.out.println( MT19937.Cauchy(0,1) );
		System.out.println( MT19937.Cauchy(0,1) );
		System.out.println( MT19937.Cauchy(0,1) );
		System.out.println( MT19937.Cauchy(0,1) );
		System.out.println( MT19937.Cauchy(0,1) );
